 ...
 *
 * @package    localwsanalyticalsystem
...
class wsanalyticalsystem_external extends external_api {
    ...
    public static function ping_parameters() {
        return new external_function_parameters(
                array('welcomemessage' => new external_value(PARAM_TEXT, 'The welcome message ', VALUE_DEFAULT, ''))
        );
    }
     ...
     */
    public static function ping($welcomemessage = 'Hello world, ') {
        global $USER;

        //Parameter validation
        //REQUIRED
        $params = self::validate_parameters(self::ping_parameters(),
     ...
     * @return external_function_parameters
     */
    public static function question_list_by_courses_parameters() {
        return new external_function_parameters(
	...
	/**
     * Returns question list by courses
     * @return string welcome message
     */
    public static function question_list_by_courses($courseId = 0) {
        global $CFG, $PAGE, $OUTPUT, $USER, $DB;

        //Parameter validation
        //REQUIRED
        $params = self::validate_parameters(self::question_list_by_courses_parameters(),
        ...
        if(IntVal($course->id) > 0){
        	...
        	$contexts = $DB->get_records('context', array('contextlevel' => CONTEXT_COURSE, 'instanceid' => $params['courseId']), 'id');

        	foreach($contexts as $context){
			    if(IntVal($context->id) > 0){
		        	$categories = $DB->get_records('question_categories', array('contextid' => $context->id), 'id');
		        	foreach($categories as $category){
			        	$questions = $DB->get_records('question', array('category' => $category->id), 'id');
			        	foreach($questions as $question){
			        		$result[] = [
				        		'id' => $question->id,
				        		'courseid' => $params['courseId'],
				        		'name' => strip_tags(str_replace("&nbsp;", " ", htmlspecialchars_decode($question->questiontext))),
     ...
     * @return external_description
     */
    public static function question_list_by_courses_returns() {
    	return new external_multiple_structure(
            new external_single_structure(
                array(
                	'id' => new external_value(PARAM_INT, 'question record id'),
                	'courseid' => new external_value(PARAM_INT, 'id of course'),
                	'name' => new external_value(PARAM_RAW, 'question name'),
                	'description' => new external_value(PARAM_RAW, 'question description text'),
                )
            )
        );
    }
     ...
     * @since Moodle 3.1
     */
    public static function quizzes_by_courses_parameters() {
        return new external_function_parameters (
            array(
     ...
     * @since Moodle 3.1
     */
    public static function quizzes_by_courses($courseids = array()) {
        global $USER;
        ...
        $params = self::validate_parameters(self::quizzes_by_courses_parameters(), $params);
     ...
     * @since Moodle 3.1
     */
    public static function quizzes_by_courses_returns() {
    ...
    /**
     * Describes the parameters for user_attempts.
     *
     * @return external_function_parameters
     * @since Moodle 3.1
     */
    public static function user_attempts_parameters() {
        return new external_function_parameters (
            array(
     ...
     */
    public static function user_attempts($quizid, $userid = 0, $status = 'finished', $includepreviews = false) {
        global $DB, $USER;
        ...
        $params = self::validate_parameters(self::user_attempts_parameters(), $params);
    ...
    /**
     * Describes the user_attempts return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function user_attempts_returns() {
        return new external_single_structure(
            array(
     ...
     * @since Moodle 3.1
     */
    public static function attempt_review_parameters() {
        return new external_function_parameters (
            array(
                'attemptid' => new external_value(PARAM_INT, 'attempt id'),
                'page' => new external_value(PARAM_INT, 'page number, empty for all the questions in all the pages', VALUE_DEFAULT, -1),
            )
        );
    }
     ...
     */
    public static function attempt_review($attemptid, $page = -1) {
        global $PAGE;
        ...
        $params = self::validate_parameters(self::attempt_review_parameters(), $params);
        ...
        $result['questions'] = self::attempt_questions_data($attemptobj, true, $page, true);
    ...
    /**
     * Describes the attempt_review return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function attempt_review_returns() {
        return new external_single_structure(
            array(
     ...
     */
    private static function attempt_questions_data(quiz_attempt $attemptobj, $review, $page = 'all') {
		global $PAGE, $DB;
		...
		$slots = $DB->get_records('quiz_slots',
                array('quizid' => $attemptobj->get_quizid()), 'slot',
        ...
        foreach ($attemptobj->get_slots($page) as $slot) {
            ...
            $question = array(
                'slot' => $slot,
                'parentQuestionid' => (int)$slots[$slot]->questionid,
    ...
    private static function question_structure() {
        return new external_single_structure(
            array(
                ...
                'parentQuestionid' => new external_value(PARAM_INT, 'question id', VALUE_OPTIONAL),
    ...
    /**
     * Describes the parameters for pages_by_courses.
     *
     * @return external_function_parameters
     * @since Moodle 3.3
     */
    public static function pages_by_courses_parameters() {
        return new external_function_parameters (
            array(
                'courseids' => new external_multiple_structure(
                    new external_value(PARAM_INT, 'Course id'), 'Array of course ids', VALUE_DEFAULT, array()
     ...
     * @since Moodle 3.3
     */
    public static function pages_by_courses($courseids = array()) {
        ...
        $params = self::validate_parameters(self::pages_by_courses_parameters(), $params);
    ...
    /**
     * Describes the pages_by_courses return value.
     *
     * @return external_single_structure
     * @since Moodle 3.3
     */
    public static function pages_by_courses_returns() {
        return new external_single_structure(
            array(
     ...
     */
    public static function enrolled_users($courseid, $options = array()) {
        global $CFG, $USER, $DB;
        ...
        $params = self::validate_parameters(
            self::enrolled_users_parameters(),
            array(
                'courseid'=>$courseid,
                'options'=>$options
            )
